...
function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("https://theory-of-computation.onrender.com/get_all_song").then(response => response.json()),
      fetch("https://theory-of-computation.onrender.com/get_all_artist").then(response => response.json()),
    ])
  ...
  const handleArtistClick = (artistName) => {
    if (!searchKeyword.trim()) {
      ...
      fetch(`https://theory-of-computation.onrender.com/get_artist/${encodeURIComponent(formattedArtistName)}`)
        .then(response => response.json())
        .then(data => {
        ...
        <div className="flex flex-col space-y-6 mb-8 overflow-hidden">
          <div className="bg-gray-200 p-4 rounded-lg ml-2 -mt-1 h-40 overflow-auto">
            ...
            <div className="flex items-center space-x-2 mb-6">
              ...
              <button className="font-semibold text-lg text-black" onClick={() => window.location.reload()}>
                Home
              </button>
            </div>
